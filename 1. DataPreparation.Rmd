---
title: "Spatial and environmental data preparation"
...
```{r packages and functions, include=T,echo=F,message=F,warning=F,results='hide'}
# 0. Load the packages----
...
```{r Download the spatial information, echo=F,include=T,message=T,warning=F}
...
spp_x <- "Macropus rufogriseus"
tax_sp <- Spatial_spp(sci_sp = spp_x,IUCN_api=key.IUCN,start_date = 2020) 
...
The algorithm was able to identified `r tax_sp$IUCN_N_syn` different synonyms. Therefore, for *`r spp_x`* the algorithm would check the **GBIF** servers for spatial data related with the species and its associated synonyms (*`r print(paste0(tax_sp$ITIS_syn %>% gsub(pattern=";",replacement=","),tax_sp$IUCN_syn %>% gsub(pattern=";",replacement=","),collapse=","))`*) and group all the observations under a single species binomial designation (the original name provided to `Spatial_spp`).
...
```{r prepare some reference polygons, echo=F,include=T,message=F, warning=F}
