...
Auto_maxent<-function(
                      ...
                      predictors, # [RAST] rasterstack object with the environmental or predictor variables to use
                      ...
                      prop_env = 0.25, # [NUMERIC] 0-1 If n_bk==AUTO, the proportion of environmental area to sample
  ...
  # a. Number of models to keep, it n.mods is set to be greather than n.m, set the first one to n.m
  if(n.mods > n.m){
    n.mods <- n.m
  }
  ...
  # Prepare the data.frame with the train and test data
  if(time_macth){
      stop("Sorry, the `time_macth==TRUE` macthing tool is not implemented at this moment! but is something we are working on!")  
